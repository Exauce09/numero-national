...
import { getSession, homeForAccountType, login } from "../auth";

const ACCOUNT_TYPES = [
  { value: "COMMUNE", label: "Communale" },
  { value: "MINISTERE", label: "Ministère" },
  { value: "AUTRES", label: "Autre Organisation" },
  { value: "CIVIL", label: "État civil" },
  { value: "AGENT", label: "Agent de Recensement" },
  { value: "ADMIN", label: "Direction de la Population" },
  { value: "PRESIDENCY", label: "Présidence" },
...
export default function LoginPage() {
  ...
  if (existing) return <Navigate to={homeForAccountType(existing.accountType)} replace />;
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setBusy(true);
    try {
      const session = await login(username, password, accountType);
      navigate(homeForAccountType(session.accountType), { replace: true });
    } catch (err) {
