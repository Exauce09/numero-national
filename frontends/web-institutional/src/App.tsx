...
import { clearSession, getSession, updatePresPassword } from "./auth";
import {
  applyPresTheme,
  getPresPrefs,
  listPresNotifications,
  markAllPresNotificationsRead,
  markPresNotificationRead,
  presUnreadCount,
  savePresPrefs,
  type PresNotification,
  type PresPrefs,
} from "./presPrefs";
...
function Shell() {
  ...
  const [prefs, setPrefs] = useState<PresPrefs>(() => getPresPrefs());
  const [notifs, setNotifs] = useState<PresNotification[]>(() => listPresNotifications());
  ...
  function persistPrefs(next: PresPrefs) {
    savePresPrefs(next);
    setPrefs(next);
  }
  ...
  function onChangePassword(e: FormEvent) {
    ...
    try {
      updatePresPassword(currentPwd, newPwd);
      setCurrentPwd("");
      setNewPwd("");
      setConfirmPwd("");
    ...
    <div className={`page-wrapper${navOpen ? " nav-open" : ""}`}>
      ...
      <aside className="sidebar" id="app-sidebar">
        <div className="sidebar-brand">
          ...
          <span>E-GOUV · Vue nationale</span>
        ...
        <nav className="sidebar-nav">
          ...
          <NavLink to="/population" onClick={() => setNavOpen(false)}>
            Population
          </NavLink>
          <NavLink to="/etat-civil" onClick={() => setNavOpen(false)}>
            États civils
          </NavLink>
          <NavLink to="/structures" onClick={() => setNavOpen(false)}>
            Structures sanitaires
          </NavLink>
          <NavLink to="/naissances" onClick={() => setNavOpen(false)}>
            Naissances
          </NavLink>
          <NavLink to="/deces" onClick={() => setNavOpen(false)}>
            Décès
          </NavLink>
          <NavLink to="/mariages" onClick={() => setNavOpen(false)}>
            Mariages
          </NavLink>
          <NavLink to="/divorces" onClick={() => setNavOpen(false)}>
            Divorces
          </NavLink>
        </nav>
      ...
      <div className="body-wrap">
        <header className="topbar topbar-3">
          <div className="topbar-left">
            ...
            <h1 className="topbar-title">Présidence de la République</h1>
          </div>
          <div className="topbar-center">
            <span className="topbar-role">Institution Présidence</span>
            <strong className="topbar-responsable">Supervision nationale</strong>
          </div>
          <div className="topbar-right">
            <button
              ...
              onClick={() => {
                ...
                setNotifs(listPresNotifications());
              }}
            >
        ...
        <main className="shell">
          <Routes>
            ...
            <Route path="/etat-civil" element={<CivilOfficesPage />} />
            <Route path="/structures" element={<FacilitiesPage />} />
            <Route path="/naissances" element={<BirthsPage />} />
            <Route path="/deces" element={<DeathsPage />} />
            <Route path="/mariages" element={<MarriagesPage />} />
          ...
          <div className="modal-panel modal-wide" onClick={(e) => e.stopPropagation()}>
            <div className="panel-head">
              ...
              <div className="modal-actions" style={{ margin: 0 }}>
                <button type="button" className="btn-secondary btn-sm" onClick={() => setNotifs(markAllPresNotificationsRead())}>
                  Tout marquer lu
                </button>
            ...
            {notifs.map((n) => (
              <article key={n.id} className={`notif-item${n.read ? "" : " unread"}`}>
                ...
                <div className="table-actions">
                  {!n.read ? (
                    <button type="button" className="btn-secondary btn-sm" onClick={() => setNotifs(markPresNotificationRead(n.id))}>
                      Marquer lu
                    </button>
                  ) : null}
                  {n.href ? (
                    <button
                      ...
                      onClick={() => {
                        setNotifs(markPresNotificationRead(n.id));
                        setNotifOpen(false);
                        navigate(n.href!);
                      }}
                    >
